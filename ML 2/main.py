...
CHANGES IN v3.0.0:
    ...
    - SensorReading accepts 4 hardware parameters: temperature, ph, nitrite (serving as Total Nitrogen), phosphorus
    - Response returns quality_level (0/1/2), quality_label (Excellent/Good/Poor)
    - NPK Adaptation: Nitrogen (N) from 7-in-1 sensor replaces Nitrite limits (now 0-200 mg/L)
...
app = FastAPI(
    ...
    description=(
        ...
        "Accepts 7-in-1 NPK sensor readings (Temp, pH, P, and Nitrogen substituted as Nitrite) and "
...
class SensorReading(BaseModel):
    """
    Four sensor readings from the 7-in-1 NPK + hardware.
    Note: Nitrogen (N) is passed into the 'nitrite' field and scaled down
    ...
    nitrite: float = Field(
        ...,
        ge=0.0, le=200.0,
        description="Nitrogen (N) concentration from 7-in-1 sensor masquerading as Nitrite (mg/kg or mg/L)",
        example=45.0,
    )
...
def predict(reading: SensorReading):
    ...
    Accepts 4 NPK/hardware sensor readings (where Nitrogen is passed as Nitrite)
    and returns a complete water quality assessment: Excellent, Good, or Poor.
    
    Warning: Nitrogen is actively scaled down during prediction to match the 
    underlying AWD Nitrite model assumptions.
    """
    try:
